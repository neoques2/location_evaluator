...
Examples:
  ...
  python main.py --dry-run --config config/analysis.yaml
    ...
    parser.add_argument(
        '--grid-size',
        type=float,
        help='Override grid size from config (miles)'
    )
    
    parser.add_argument(
        '--max-radius',
        type=float,
        help='Override max radius from config (miles)'
    )
...
def show_analysis_plan(config: Dict[str, Any], output_path: str) -> None:
    ...
    import math
    estimated_points = math.pi * (analysis_config['max_radius'] / analysis_config['grid_size']) ** 2

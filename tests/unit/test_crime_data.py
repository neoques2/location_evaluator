...
def test_get_crime_data_uses_cache(monkeypatch, tmp_path):
    ...
    result1 = crime_data.get_crime_data(
        40.0, -75.0, radius_miles=0.5, cache_duration_days=1
    )
    assert calls["count"] == 1

    result2 = crime_data.get_crime_data(
        40.0, -75.0, radius_miles=0.5, cache_duration_days=1
    )
...
def test_get_crime_data_large_bbox():
    lat, lon = 32.7767, -96.7970
    radius_miles = 34.5  # roughly 0.5 degrees latitude
    captured = {}

    result = crime_data.get_crime_data(
        lat, lon, radius_miles=radius_miles, use_cache=False
    )

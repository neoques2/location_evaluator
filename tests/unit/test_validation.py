...
def _create_config(tmp_path: Path):
  ...
  work:
    ...
    (tmp_path / "transportation.yaml").write_text(
        "transportation:\n  modes:\n    - driving\n"
    )
...
def test_geocoding_validation(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.config_parser.geocode_address", lambda addr, **k: {"lat": 1.0, "lon": 2.0}
    )
    monkeypatch.setattr(
        "src.config_parser.check_network_connectivity", lambda url, timeout=5: True
    )
...
def test_network_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "src.config_parser.check_network_connectivity", lambda url, timeout=5: False
    )
    monkeypatch.setenv("LE_SKIP_NETWORK", "0")

...
class AnalysisGrid:
    ...
    def __init__(self, center_lat: float, center_lon: float, radius_miles: float, grid_size_miles: float):
        """
    ...
    def _haversine_vectorized(self, lat1: np.ndarray, lon1: np.ndarray, 
    ...
    def add_column(self, name: str, values: np.ndarray) -> None:
        ...
        if len(values) != len(self.grid_df):
            raise ValueError(f"Values length {len(values)} doesn't match grid length {len(self.grid_df)}")

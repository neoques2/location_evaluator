...
class GridExplorer:
    ...
    def create_grid_overview_map(
        self, title: str = "Analysis Grid Overview"
    ) -> go.Figure:
        """
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                marker=dict(
                    ...
                    colorscale="Viridis",
                    colorbar=dict(
                        title="Distance from Center (miles)", thickness=15, len=0.7
                    ),
                    opacity=0.7,
                ),
                text=[
        ...
        fig.add_trace(
            go.Scattermapbox(
                ...
                text=[
                    f"Analysis Center<br>Lat: {self.grid.center_lat:.4f}<br>Lon: {self.grid.center_lon:.4f}"
                ],
                hovertemplate="%{text}<extra></extra>",
                name="Center Point",
            )
        ...
        if self.destinations_df is not None:
            ...
            sample_destinations = [
                {
                    "name": "Work",
                    "lat": self.grid.center_lat + 0.1,
                    "lon": self.grid.center_lon + 0.1,
                    "category": "work",
                },
                {
                    "name": "Gym",
                    "lat": self.grid.center_lat - 0.05,
                    "lon": self.grid.center_lon + 0.05,
                    "category": "personal",
                },
                {
                    "name": "Doctor",
            ...
            for category in dest_df["category"].unique():
                ...
                fig.add_trace(
                    go.Scattermapbox(
                        ...
                        text=[
                            f"{row['name']}<br>Category: {row['category']}"
        ...
        fig.update_layout(
            ...
            mapbox=dict(
                ...
                center=dict(lat=self.grid.center_lat, lon=self.grid.center_lon),
                zoom=9,
            ),
            height=700,
            showlegend=True,
    ...
    def create_grid_statistics_dashboard(self) -> go.Figure:
        ...
        fig.add_trace(
            go.Histogram(
                x=grid_df["lat"], nbinsx=20, name="Latitude", marker_color="lightgreen"
            ),
            row=1,
            col=2,
        )
        ...
        fig.add_trace(
            go.Histogram(
                x=grid_df["lon"], nbinsx=20, name="Longitude", marker_color="lightcoral"
            ),
            row=2,
            col=1,
        )
        ...
        # Update layout
        fig.update_layout(title="Grid Analysis Dashboard", height=800, showlegend=False)
...
def visualize_grid_from_config(
    config_path: str = "config", output_file: str = "outputs/grid_visualization.html"
) -> None:
    """

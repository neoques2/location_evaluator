...
class ConfigParser:
    ...
    def _load_modular_config(self, config_dir: Path) -> Dict[str, Any]:
        ...
        for filename, section in config_files.items():
            ...
            if not file_path.exists():
                raise ConfigValidationError(f"Required config file missing: {file_path}")
            ...
            try:
                ...
                if not isinstance(file_config, dict):
                    raise ConfigValidationError(f"Config file {filename} must contain a dictionary")
    ...
    def _validate_analysis_config(self, analysis: Dict[str, Any]) -> None:
        ...
        if isinstance(center_point, str):
            ...
            raise ConfigValidationError("Center point must be address string or [lat, lon] coordinates")
    ...
    def _validate_destinations_config(self, destinations: Dict[str, Any]) -> None:
        ...
        for category, dest_list in destinations.items():
            if not isinstance(dest_list, list):
                raise ConfigValidationError(f"Destination category '{category}' must be a list")
            
            for i, dest in enumerate(dest_list):
                if not isinstance(dest, dict):
                    raise ConfigValidationError(f"Destination {i} in '{category}' must be a dictionary")
                
                # Required fields
                if 'address' not in dest:
                    raise ConfigValidationError(f"Destination {i} in '{category}' missing address")
                if 'name' not in dest:
                    raise ConfigValidationError(f"Destination {i} in '{category}' missing name")
                if 'schedule' not in dest:
                    raise ConfigValidationError(f"Destination {i} in '{category}' missing schedule")
    ...
    def _validate_schedule(self, schedule: list, dest_name: str) -> None:
        ...
        if not isinstance(schedule, list) or len(schedule) == 0:
            raise ConfigValidationError(f"Schedule for {dest_name} must be a non-empty list")
        
        for i, schedule_item in enumerate(schedule):
            if not isinstance(schedule_item, dict):
                raise ConfigValidationError(f"Schedule item {i} for {dest_name} must be a dictionary")
            
            # Must have arrival_time
            if 'arrival_time' not in schedule_item:
                raise ConfigValidationError(f"Schedule item {i} for {dest_name} missing arrival_time")
            
            # Validate time format (HH:MM)
            arrival_time = schedule_item['arrival_time']
            if not self._validate_time_format(arrival_time):
                raise ConfigValidationError(f"Invalid time format '{arrival_time}' in {dest_name}. Use HH:MM format")
            ...
            if 'days' in schedule_item:
                ...
                if not isinstance(days, str):
                    raise ConfigValidationError(f"Days field in {dest_name} must be a string")
                
                # Validate day patterns
                if not self._validate_days_pattern(days):
                    raise ConfigValidationError(f"Invalid days pattern '{days}' in {dest_name}")
            
            elif 'pattern' in schedule_item:
                # Monthly pattern
                pattern = schedule_item['pattern']
                if not isinstance(pattern, str):
                    raise ConfigValidationError(f"Pattern field in {dest_name} must be a string")
                
                # Validate monthly patterns
                if not self._validate_monthly_pattern(pattern):
                    raise ConfigValidationError(f"Invalid monthly pattern '{pattern}' in {dest_name}")
            
            else:
                raise ConfigValidationError(f"Schedule item {i} for {dest_name} must have 'days' or 'pattern'")
    ...
    def _validate_output_config(self, output: Dict[str, Any]) -> None:
        ...
        if 'output_format' in output:
            ...
            if output['output_format'] not in valid_formats:
                raise ConfigValidationError(f"Invalid output format: {output['output_format']}")

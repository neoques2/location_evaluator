            ...
            try:
                ...
                if e.response.status_code == 429:  # Rate limit exceeded
                    if attempt == self.retry_count - 1:
                        raise RateLimitError(f"Rate limit exceeded after {self.retry_count} attempts")
                    wait_time = self.backoff_factor ** attempt
                    self.logger.warning(f"Rate limit hit (attempt {attempt + 1}), retrying in {wait_time} seconds...")
                    ...
                    if attempt == self.retry_count - 1:
                        raise
                    wait_time = self.backoff_factor ** attempt
                    self.logger.warning(f"HTTP error {e.response.status_code} (attempt {attempt + 1}), retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                ...
                if attempt == self.retry_count - 1:
                    raise
                wait_time = self.backoff_factor ** attempt
                self.logger.warning(f"Network error (attempt {attempt + 1}), retrying in {wait_time} seconds...")
                time.sleep(wait_time)
                ...
                if attempt == self.retry_count - 1:
                    raise
                wait_time = self.backoff_factor ** attempt
                self.logger.warning(f"API call failed (attempt {attempt + 1}), retrying in {wait_time} seconds...")
                time.sleep(wait_time)

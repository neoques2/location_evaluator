...
class HTMLReportGenerator:
    ...
    def save_plotly_figure(self, fig: go.Figure, filename: str) -> str:
        ...
        print(f"{AnalysisConstants.SUCCESS_EMOJI} Saved: {file_path} ({file_size:.1f} KB)")
...
"""
    ...
    def create_complete_report(
        ...
        supporting_path = self.save_plotly_figure(supporting_figure, supporting_filename)
        ...
        print(f"{AnalysisConstants.SUCCESS_EMOJI} Complete report saved: {combined_path} ({combined_size:.1f} KB)")
    ...
    def print_analysis_summary(self, analysis_result: GridAnalysisResult) -> None:
        ...
        print(f"{AnalysisConstants.SUCCESS_EMOJI} Best Location (Lowest Weekly Travel Time):")
        print(f"   Point {best_location['point_id']}: ({best_location['lat']:.4f}, {best_location['lon']:.4f})")
        print(f"   Total weekly travel time: {best_location['total_weekly_travel_time']:.1f} minutes")
        ...
        print(f"   Point {worst_location['point_id']}: ({worst_location['lat']:.4f}, {worst_location['lon']:.4f})")
        print(f"   Total weekly travel time: {worst_location['total_weekly_travel_time']:.1f} minutes")
        ...
        print(f"   Average weekly travel time: {stats['mean_travel_time']:.1f} ± {stats['std_travel_time']:.1f} min")
        
        print(f"\n{AnalysisConstants.TARGET_EMOJI} Weekly Schedule Details for Best Location:")
        for route in best_location["route_details"]:
            print(f"   → {route['destination']}: {route['travel_time']:.1f} min/trip × {route['frequency']} = {route['weekly_time']:.1f} min/week")
...
class ReportFileManager:
    ...
    def cleanup_old_reports(self, report_prefix: str) -> None:
        ...
        patterns = [
            f"{report_prefix}_*.html",
            f"{report_prefix}_complete.html"
        ]
